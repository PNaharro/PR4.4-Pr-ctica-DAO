...
public class DaoLlenguatge implements Dao<ObjLlenguatge> {
    ...
    private void writeList(ArrayList<ObjLlenguatge> llista) {
        try {
            ...
            PrintWriter out = new PrintWriter(new FileWriter(Main.llenguatgesPath));
    ...
    public void print() {
        ...
        for (int cnt = 0; cnt < llista.size(); cnt = cnt + 1) {
            ObjLlenguatge llenguatge = llista.get(cnt);
            System.out.println("Llenguatge: " + llenguatge.getId() + " " + llenguatge.getNom() + ", " +
                    llenguatge.getAny() + " - " + llenguatge.getDificultat() + "/" + llenguatge.getPopularitat());
        }
    }
