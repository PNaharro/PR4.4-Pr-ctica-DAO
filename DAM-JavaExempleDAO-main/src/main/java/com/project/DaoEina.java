...
public class DaoEina implements Dao<ObjEina> {
    ...
    private void writeList(ArrayList<ObjEina> llista) {
        try {
            ...
            PrintWriter out = new PrintWriter(new FileWriter(Main.einesPath));
    ...
    public void setLlenguatgesAdd(int idEina, int idLlenguatge) {
        ArrayList<ObjEina> llista = getAll();
        int pos = getPosition(idEina);
        if (pos != -1) {
            ObjEina eina = llista.get(pos);
            if (!eina.getLlenguatges().contains(idLlenguatge)) {
    ...
    public void setLlenguatgesDelete(int idEina, int idLlenguatge) {
        ArrayList<ObjEina> llista = getAll();
        int pos = getPosition(idEina);
        if (pos != -1) {
            ObjEina eina = llista.get(pos);
    ...
    public void print() {
        ArrayList<ObjEina> llista = getAll();
        for (int cnt = 0; cnt < llista.size(); cnt = cnt + 1) {
            System.out.println("Eina: " + llista.get(cnt));
        }
    }
}
